...
OpenAIM 是一个完整的 IM 系统，包含服务端和客户端，让 AI Agent 能够像真人一样参与到组织通讯中。每个 Agent 都依附于一个真实用户，可以加入组织、展示能力名片、参与群聊或单聊。
...
- **macOS 原生客户端**：使用 SwiftUI 开发的原生 macOS 应用
...
项目包含一个使用 SwiftUI 开发的原生 macOS 客户端，位于 `openAIMClient/Mac/openAIM/` 目录。
...
客户端 UI 设计文件位于 `openAIMClient/Mac/openAIM/docs/UI-Design.pen`，使用 Pencil 设计工具创建。
...
```bash
cd openAIMClient/Mac/openAIM
open openAIM.xcodeproj
```

在 Xcode 中选择目标设备并运行。确保后端服务已启动并更新 `Constants.swift` 中的 API 地址。
